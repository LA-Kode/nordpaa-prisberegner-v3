(function(){
  ...
  const state = JSON.parse(JSON.stringify(npPBv3Admin.config||{}));
  if(!state.bands) state.bands=[{label:'1-25',min:1,max:25}];
  ...
  function render(){
    const app=document.getElementById('np3-admin-app'); app.innerHTML='';
    const header=el('div',{class:'np3-admin-header'},[
      el('div',{class:'np3-tabs'},[tab('general','Generelt'),tab('bands','Ansattegrupper'),tab('modules','Moduler'),tab('branding','Branding'),tab('pdf','PDF Generator')]),
      el('div',{class:'np3-actions'},[btn('Nulstil til demo','ghost',reset),btn('Gem ændringer','primary',save)])
    ]);
    const body=el('div',{class:'np3-admin-body'});
    app.appendChild(header); app.appendChild(body);
    show('general');
  }
  ...
  function general(body){
    const w=el('div',{class:'np3-card'});
    w.appendChild(el('h3',{},'Generelle indstillinger'));

    const r1=el('div',{class:'np3-row'},[
      el('div',{class:'np3-label'},'Primær farve'),
      el('div',{class:'np3-ctrl'},[color(state.color), input(state.color,{class:'np3-text'})])
    ]);
    const c=r1.querySelector('input[type=color]');
    const t=r1.querySelector('.np3-text');
    c.value = state.color || '#0B2650';
    t.value = state.color || '#0B2650';
    ...
    const r2=el('div',{class:'np3-row'},[
      el('div',{class:'np3-label'},'Årlig rabat (%)'),
      el('div',{class:'np3-ctrl'},[number(state.yearly_discount||0)])
    ]);
    r2.querySelector('input').addEventListener('input',e=> state.yearly_discount=parseFloat(e.target.value||0));

    [r1,r2].forEach(x=>w.appendChild(x));
    body.appendChild(w);
  }
  ...
  function bands(body){
    ...
    function row(b,i){
      ...
      [l,mi,ma,del].forEach(x=>r.appendChild(x));
      return r;
    }
  }
  ...
  async function save(){
    ...
    toast(data && data.success ? 'Gemt ✔' : 'Fejl ved gemning');
  }
  ...
  function toast(msg){ let t=document.querySelector('.np3-toast'); if(!t){ t=el('div',{class:'np3-toast'}); document.body.appendChild(t);} t.textContent=msg; t.classList.add('show'); setTimeout(()=>t.classList.remove('show'),1600); }
